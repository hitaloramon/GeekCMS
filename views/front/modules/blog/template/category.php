<?php if(isset($url[1])): ?>
    ...
    <?php $posts = $blog->getPostByCategory($url[1], $start, $resultsPerPage); if(!empty($posts)): ?>
            ...
            <div class="row m-b-30">
                <?php if(!empty($post['thumb'])): ?>
                    <div class="col-md-4 mb-3">
                        <a href="<?php echo BASE.'/'. $post['blogslug']. '/visualizar/'. $post['slug']; ?>"><img class="img-responsive rounded" src="<?php echo BASE_UPLOADS; ?>/<?php echo $post['thumb']; ?>"></a>
                    </div>
                <?php endif; ?>
                <div class="col-md">
                    <a href="<?php echo BASE.'/'. $post['blogslug']. '/visualizar/'. $post['slug']; ?>">
        ...
        <?php endforeach; ?>
        <?php 
            $link = BASE.'/'. $post['blogslug'].'/categoria/'. $url[1] . '/';
